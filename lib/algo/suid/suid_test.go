...
const (
	errMsg = "error creating NewNode, %s"
	errPo  = "pID %v != oID %v"
	errEp  = "error parsing, %s"
)
...
func TestMarshalJSON(t *testing.T) {
	...
	xbytes, err := id.MarshalJSON()
	if err != nil {
		t.Fatalf("Unexpected error during MarshalJSON")
	}

	if string(xbytes) != expected {
		t.Fatalf("Got %s, expected %s", string(xbytes), expected)
	}
}
