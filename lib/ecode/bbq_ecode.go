...
var (
	...
	UserNotExists     = New(5005102) // 用户不存在
	GetUserBaseErr    = New(5005103) // 获取用户信息失败
	EditUserBaseErr   = New(5005104) // 更新用户基础信息失败
	UserNameSpecial   = New(5005105) // 昵称包含特殊字符
	UserNameLength    = New(5005106) // 昵称长度不符合
	UserNameExisted   = New(5005107) // 昵称已被占用
	UserNameFilterErr = New(5005108) // 昵称包含敏感词
	UserNamePrefixErr = New(5005109) // 该昵称无法注册
