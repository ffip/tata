...
func TestMySQL(t *testing.T) {
	...
	conf := `{"mysql":{"addr":"test","dsn":"root:test@tcp(localhost:3306)/troy?timeout=5s&readTimeout=5s&writeTimeout=5s&parseTime=true&loc=Local&charset=utf8,utf8mb4","readDSN":["root:test@tcp(localhost:3306)/troy?timeout=5s&readTimeout=5s&writeTimeout=5s&parseTime=true&loc=Local&charset=utf8,utf8mb4"],"archive":20,"idle":10,"idleTimeout":"4h","queryTimeout":"15s","execTimeout":"5s","tranTimeout":"5s"}}`

	s := &Config{}
	dsn := gjson.Get(conf, "mysql").Raw
	_ = json.Unmarshal([]byte(dsn), &s)
	s.Breaker = bc
	db := NewMySQL(s)
