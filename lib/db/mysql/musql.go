...
// ToMap - run sql and return an array of maps
func (rs *Rows) ToMap() (result []map[string]interface{}) {
...
// ToJSONStr  - run sql and return an JSON of string.
func (rs *Rows) ToJSONStr() (result string) {
	jsonStr, err := json.Marshal(rs.ToMap())
	if err != nil {
		fmt.Println("MapToJsonDemo err: ", err)
		return ``
	}
	return text.Abot(jsonStr)
}
...
func (db *conn) begin(c context.Context) (tx *Tx, err error) {
	...
	if err = db.breaker.Allow(); err != nil {
		fmt.Println("mysql:begin", "breaker")
		return
	}
	_, c, cancel := db.conf.TranTimeout.Shrink(c)
	rtx, err := db.BeginTx(c, nil)
	fmt.Println("mysql:begin", int64(time.Since(now)/time.Millisecond))
	if err != nil {
		err = errors.WithStack(err)
		cancel()
		return
...
func (db *conn) exec(c context.Context, query string, args ...interface{}) (res sql.Result, err error) {
	...
	if err = db.breaker.Allow(); err != nil {
		fmt.Println("mysql:end", "breakerexec", "breaker")
		return
	}
	...
	cancel()
	db.onBreaker(&err)
	fmt.Println("mysql:exec", int64(time.Since(now)/time.Millisecond))
	if err != nil {
...
func (db *conn) ping(c context.Context) (err error) {
	...
	if err = db.breaker.Allow(); err != nil {
		fmt.Println("mysql:end", "breakerping", "breaker")
		return
	}
	_, c, cancel := db.conf.ExecTimeout.Shrink(c)
	...
	cancel()
	db.onBreaker(&err)
	fmt.Println("mysql:ping", int64(time.Since(now)/time.Millisecond))
	if err != nil {
		err = errors.WithStack(err)
	}
	return
...
func (db *conn) query(c context.Context, query string, args ...interface{}) (rows *Rows, err error) {
	...
	if err = db.breaker.Allow(); err != nil {
		fmt.Println("mysql:end", "breakerquery", "breaker")
		return
	}
	_, c, cancel := db.conf.QueryTimeout.Shrink(c)
	rs, err := db.DB.QueryContext(c, query, args...)
	db.onBreaker(&err)
	fmt.Println("mysql:query", int64(time.Since(now)/time.Millisecond))
	if err != nil {
...
func (db *conn) queryRow(c context.Context, query string, args ...interface{}) *Row {
	...
	if err := db.breaker.Allow(); err != nil {
		fmt.Println("mysql:end", "breakerqueryrow", "breaker")
		return &Row{db: db, t: t, err: err}
	}
	_, c, cancel := db.conf.QueryTimeout.Shrink(c)
	r := db.DB.QueryRowContext(c, query, args...)
	fmt.Println("mysql:queryrow", int64(time.Since(now)/time.Millisecond))
...
func (s *Stmt) Exec(c context.Context, args ...interface{}) (res sql.Result, err error) {
	...
	if err = s.db.breaker.Allow(); err != nil {
		fmt.Println("mysql:end", "breakerstmt:exec", "breaker")
		return
	}
	stmt, ok := s.stmt.Load().(*sql.Stmt)
	if !ok {
	...
	cancel()
	s.db.onBreaker(&err)
	fmt.Println("mysql:stmt:exec", int64(time.Since(now)/time.Millisecond))
	if err != nil {
...
func (s *Stmt) Query(c context.Context, args ...interface{}) (rows *Rows, err error) {
	...
	if err = s.db.breaker.Allow(); err != nil {
		fmt.Println("mysql:end", "breakerstmt:query", "breaker")
		return
	}
	stmt, ok := s.stmt.Load().(*sql.Stmt)
	if !ok {
	...
	s.db.onBreaker(&err)
	fmt.Println("mysql:stmt:query", int64(time.Since(now)/time.Millisecond))
	if err != nil {
...
func (s *Stmt) QueryRow(c context.Context, args ...interface{}) (row *Row) {
	...
	if row.err = s.db.breaker.Allow(); row.err != nil {
		fmt.Println("mysql:end", "breakerstmt:queryrow", "breaker")
		return
	}
	stmt, ok := s.stmt.Load().(*sql.Stmt)
	if !ok {
	...
	row.cancel = cancel
	fmt.Println("mysql:stmt:queryrow", int64(time.Since(now)/time.Millisecond))
	return
}
...
func (tx *Tx) Exec(query string, args ...interface{}) (res sql.Result, err error) {
	...
	fmt.Println("mysql:tx:exec", int64(time.Since(now)/time.Millisecond))
	if err != nil {
		err = errors.Wrapf(err, "exec:%s, args:%+v", query, args)
	}
	return
...
func (tx *Tx) Query(query string, args ...interface{}) (rows *Rows, err error) {
	...
	now := time.Now()
	defer func() {
		fmt.Println("mysql:tx:query", int64(time.Since(now)/time.Millisecond))
	}()
...
func (tx *Tx) QueryRow(query string, args ...interface{}) *Row {
	...
	now := time.Now()
	defer func() {
		fmt.Println("mysql:tx:queryrow", int64(time.Since(now)/time.Millisecond))
	}()
